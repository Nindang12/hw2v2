...
# CẤU HÌNH HỆ THỐNG
# ===========================
KAFKA_BOOTSTRAP = "192.168.80.127:9092"
...
# ===========================
# STATEFUL AGGREGATION - FIX ĐÚNG RỒI
# ===========================
...
# FIX: Dùng min() để giữ entry_time across batches (stateful!)
parking_aggregated = watermarked_df.groupBy(
    col("location"),
    col("license_plate")
).agg(
    last("status_code").alias("current_status"),
    # FIX CUỐI CÙNG: min() giữ giá trị nhỏ nhất across tất cả batches
    min(when(col("status_code") == "ENTERING", col("timestamp_unix"))).alias("entry_time"),
    min(when(col("status_code") == "ENTERING", col("timestamp"))).alias("entry_timestamp"),
...
console_df = watermarked_df.groupBy(
    ...
    last("status_code").alias("current_status"),
    # FIX: Dùng min() để giữ entry_time
    min(when(col("status_code") == "ENTERING", col("timestamp_unix"))).alias("entry_time"),
    min(when(col("status_code") == "ENTERING", col("timestamp"))).alias("entry_timestamp"),
    max("timestamp_unix").alias("last_update")
).withColumn(
...
# ===========================
print("\n" + "=" * 80)
print("🚗 PARKING FEE CALCULATOR - SPARK STREAMING (FIXED V3 - FINAL)")
print("=" * 80)
...
print("\n🔧 FIXES APPLIED:")
print("  - Dùng min() thay vì first() để giữ entry_time ACROSS BATCHES")
print("  - Entry_time giờ là STATEFUL, không bị mất khi status thay đổi")
print("  - Hiển thị tất cả status kể cả EXITING")
print("=" * 80 + "\n")
